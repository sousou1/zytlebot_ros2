...
#define RF_HOG_WIDTH 64
#define RF_HOG_HEIGHT 32
#define RF_CELL_SIZE_ROW 8
#define RF_CELL_SIZE_COL 8
#define RF_BLOCK_SIZE_ROW 2
#define RF_BLOCK_SIZE_COL 2
#define RF_N_CELLS_ROW RF_HOG_HEIGHT/RF_CELL_SIZE_ROW //4
#define RF_N_CELLS_COL RF_HOG_WIDTH/RF_CELL_SIZE_COL //8
#define RF_N_BLOCK_ROW RF_HOG_HEIGHT/(RF_CELL_SIZE_ROW * RF_BLOCK_SIZE_ROW) //2
#define RF_N_BLOCK_COL RF_HOG_WIDTH/(RF_CELL_SIZE_COL * RF_BLOCK_SIZE_COL) //4
#define RF_HISTOGRAMSIZE RF_N_CELLS_ROW * RF_N_CELLS_COL * 9 //4*8*9


inline int rf_approx_distance(int dx, int dy){
  int min, max; //uint
  if(dx < 0) dx = -dx;
  if(dy < 0) dy = -dy;
...
void rf_lite_hog(cv::Mat img, unsigned short* dst, bool approx_mode=true){
  int descriptor[RF_HISTOGRAMSIZE]={0};
  int sum_of_block[RF_N_BLOCK_ROW * RF_N_BLOCK_COL]={0};
  for(int i = 0; i < RF_HISTOGRAMSIZE; i++) descriptor[i] = 0;
  for(int i = 0; i < RF_N_BLOCK_ROW * RF_N_BLOCK_COL; i++) sum_of_block[i] = 0;
  ...
  for(int y = 0; y < RF_N_BLOCK_ROW; y++){//2
  for(int x = 0; x < RF_N_BLOCK_COL; x++){//4
    for(int yy = 0; yy < RF_CELL_SIZE_ROW*RF_BLOCK_SIZE_ROW; yy++){//16
    for(int xx = 0; xx < RF_CELL_SIZE_COL*RF_BLOCK_SIZE_ROW; xx++){//16
      int ny = (RF_BLOCK_SIZE_ROW * RF_CELL_SIZE_ROW) * y + yy;
      int nx = (RF_BLOCK_SIZE_COL * RF_CELL_SIZE_COL) * x + xx;
      //compute gradient
      int mag = 0;
      int bin_index;
      ...
      mag = (approx_mode ? (rf_approx_distance(Gx, Gy)) : sqrt(Gx*Gx + Gy * Gy));
      ...
      int block_index = y * RF_N_BLOCK_COL + x;
      int cy = yy / RF_CELL_SIZE_ROW;
      int cx = xx / RF_CELL_SIZE_COL;
  ...
  // int normalized[RF_HISTOGRAMSIZE]={0};
  // for(int i = 0; i < RF_HISTOGRAMSIZE; i++) normalized[i] = 0;
  int  average;
  unsigned int blkSize = 36;
  for(int by = 0; by < RF_N_BLOCK_ROW; by++){//2
  for(int bx = 0; bx < RF_N_BLOCK_COL; bx++){//4
    int blkIdx = by * RF_N_BLOCK_COL + bx;//0, 1.. 7
    for (int i = 0; i < 36; i++) {
    //Average of block
...
void rf_ravel(cv::Mat img, unsigned short* dst){
  vector<int> rst;
